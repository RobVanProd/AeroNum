#!/usr/bin/env python3
"""Compare AeroNum sampled GGUF output with llama.cpp sampled CLI output."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare AeroNum sampled GGUF generated text with llama.cpp CLI."
    )
    parser.add_argument("--input", required=True, help="AeroNum sampled result JSON")
    ...
    seed = int(aeronum["initial_seed"])
    temperature = float(aeronum["temperature"])
    sampling_scope = str(aeronum.get("sampling_scope", "top-k"))
    top_k = "0" if sampling_scope == "full-vocab" else str(int(aeronum["top_k"]))
    ...
    result = {
        "comparison": "llama_cpp_sampled_generated_output",
        "input": args.input,
        "model": args.model,
        "prompt": prompt,
        "predict": predict,
        ...
        "limitations": [
            "compares sampled generated text and re-tokenized IDs only",
            "does not prove identical RNG or sampler implementation internals",
