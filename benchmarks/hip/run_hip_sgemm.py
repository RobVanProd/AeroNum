#!/usr/bin/env python3
"""Compile and run the HIP/hipBLAS SGEMM benchmark for ROCm."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run HIP/hipBLAS SGEMM benchmark")
    ...
    parser.add_argument("--arch", default="gfx1101", help="GPU target arch")
...
def main() -> int:
    ...
    src = repo_root / "benchmarks" / "hip" / "sgemm_4096.hip.cpp"
    out_dir = repo_root / "benchmarks" / "results" / "hip"
    out_dir.mkdir(parents=True, exist_ok=True)
    exe = out_dir / "sgemm_4096_hip.exe"

    hipcc = find_hipcc()
    if not hipcc:
        print("hipcc not found. Install ROCm HIP SDK or set HIPCC", file=sys.stderr)
        return 2
    ...
    output_path = Path(args.output) if args.output else out_dir / f"hip_sgemm_{timestamp}.json"
