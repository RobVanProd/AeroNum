...
fn sample_json(
    ...
    let generated_token = sample.top_logits.first().expect("top token");
    let generated_piece = top_token_pieces.first().expect("top token piece");
    format!(
        concat!(
            "{{",
            "\"prompt\":\"{}\",",
...
fn main() {
    ...
    if model_path.is_empty() {
        eprintln!(
            "usage: gguf_prompt_greedy_decode_smoke --model <path> [--prompt <text>] [--layers <count>]"
        );
        std::process::exit(2);
    }
    ...
    println!(
        concat!(
            "{{",
            "\"benchmark\":\"gguf_prompt_greedy_decode_smoke\",",
            "\"model_path\":\"{}\",",
            "\"elapsed_ms\":{:.6},",
            "\"gguf_version\":{},",
            "\"tensor_count\":{},",
            ...
            "\"samples\":[{}],",
            "\"limitations\":[",
            "\"CPU one-step greedy next-token piece selection for one fixed prompt only\",",
            "\"RoPE arithmetic is internal AeroNum computation, not external parity\",",
            "\"not multi-token autoregressive generated text\",",
            "\"not GPU matmul\",",
