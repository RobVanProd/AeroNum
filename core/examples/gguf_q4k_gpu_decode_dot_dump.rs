...
fn main() {
    ...
    if model_path.is_empty() {
        eprintln!(
            "usage: gguf_q4k_gpu_decode_dot_dump --model <path> --lhs-bin <path> --rhs-q4k-bin <path>"
        );
        std::process::exit(2);
    }
    let lhs_bin = parse_arg("--lhs-bin", "");
    let rhs_q4k_bin = parse_arg("--rhs-q4k-bin", "");
    if lhs_bin.is_empty() || rhs_q4k_bin.is_empty() {
        eprintln!("--lhs-bin and --rhs-q4k-bin are required");
        std::process::exit(2);
    }
    let lhs_tensor = parse_arg("--lhs-tensor", "output.weight");
    let rhs_tensor = parse_arg("--rhs-tensor", "token_embd.weight");
    let lhs_row = parse_u64_arg("--lhs-row", 100);
    let rhs_row = parse_u64_arg("--rhs-row", 22177);
    ...
    if rhs.tensor_type != 12 {
        eprintln!("rhs tensor must be Q4_K for this verifier");
        std::process::exit(3);
    }
    ...
    write_u8_file(&rhs_q4k_bin, &rhs.row_bytes);
    ...
    println!(
        concat!(
            "{{",
            "\"benchmark\":\"gguf_q4k_gpu_decode_dot_dump\",",
            "\"model_path\":\"{}\",",
            "\"elapsed_ms\":{:.6},",
            "\"gguf_version\":{},",
            "\"tensor_count\":{},",
            ...
            "\"cpu_dot\":{:.12},",
            "\"validation\":\"dumped_decoded_lhs_and_raw_q4k_rhs\"",
            "}}"
        ),
